...
class pagedList<T> {
    ...
    private items = [];
    isFetching = false;
    queuedFetch: { skip: number; take: number; task: JQueryDeferred<pagedResultSet<T>> } = null;
    collectionName = "";
    currentItemIndex = ko.observable(0);

    constructor(private fetcher: (skip: number, take: number) => JQueryPromise<pagedResultSet<T>>) {
        if (!fetcher) {
    ...
    fetch(skip: number, take: number): JQueryPromise<pagedResultSet<T>> {
        if (this.isFetching) {
        ...
        if (cachedItemsSlice) {
            ...
            var deferred = $.Deferred<pagedResultSet<T>>();
            ...
            var remoteFetch = this.fetcher(skip, take)
                .done((resultSet: pagedResultSet<T>) => {
