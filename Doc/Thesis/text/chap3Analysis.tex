...
\todoi{operators might be better in objectives?}
...
\end{itemize}
\todoi{in operator is an extension - maybe mention later}
...
\todoi{maybe should be in objectives, requirements - doesn't make much sense to add now}
...
\todoi{add document db comparison}
...
The most human readable and easiest to consume in programs is JSON-LD, which was designed with these two goals. \todoi{source?}
...
\end{description}
\todoi{add samples for what the algorithms do}
\todoi{flat vs nested data - easier to do nested objects queries, but not a norm}
...
\todoi{now i need to query nested data, wasn't stated before}
...
\todoi{here i state that the returned data is a graph - should probably say that construct queries usually return a graph of triplets}
...
\todoi{maybe add a more formal name and replace it, to add more consistency and clearness}
\todoi{add example of the shape we want}
\todoi{give more meaning to the unflatten algorithm .. maybe define with a pseudo language}
...
\todoi{add a section about Web APIs and REST - maybe its a design choice.. }
